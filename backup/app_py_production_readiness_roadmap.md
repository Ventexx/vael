...
- **Platform matrix.** The one live-tested finding in the code (`-u<state>!<newArchiveName>` mutating the source archive) was tested against **7-Zip 23.01 on Linux only**. This is explicitly called out as unverified elsewhere. Before shipping to any Windows user (which the spec's own example paths — `D:\Documents`, etc. — clearly anticipate), the same empirical test needs to be repeated against the actual Windows 7-Zip build(s) you intend to support, since 7-Zip's Windows and POSIX builds have historically diverged in exactly this kind of flag behavior. Do not assume the Linux finding transfers.
...
### 2.3 Large-archive / long-run behavior
...
- Memory behavior of `parse_archive_listing` and `SourceInventoryManager.scan` against a source tree with hundreds of thousands of files (both build full in-memory dicts).
- Wall-clock cost of the reintroduced full staging copy on a large archive, and whether that's acceptable, since it was explicitly reintroduced as a correctness trade-off against performance.
- `preflight_space`'s headroom formula at real scale (does 10% + 50 MiB still make sense at 500 GB?).
